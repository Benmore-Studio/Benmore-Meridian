...
- `bm suggest --install --cache` keeps agent context narrow by installing only project-matched skills and persisting the recommendation set for later inspection.
- `bm uninstall --all --yes` makes cleanup fast and reversible, while preserving source skills and external registry entries.
- `bm context --global --json` gives Codex/Claude complete catalog awareness without requiring every skill to stay installed or loaded.
- JSON contracts give agents stable stdout; Rich tables remain for humans.
- Native wheels optimize hot filesystem/parsing helpers, while Python fallback keeps editable installs and source builds dependable.
- The release workflow proves both packaging modes: `uv build --sdist` for source distribution and `maturin build` plus `verify_native_wheel.py` for PyO3 wheels.
...
```mermaid
sequenceDiagram
    participant Tag as git tag v1.10.2
    ...
    CI->>CI: verify_native_wheel.py confirms bm._native is packaged
...
- Native wheel jobs fail unless the wheel contains the `bm._native` PyO3 extension.
